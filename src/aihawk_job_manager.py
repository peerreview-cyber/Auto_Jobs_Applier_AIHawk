...
class AIHawkJobManager:
    ...
    def set_parameters(self, parameters):
        ...
        self.always_tailor_resume = parameters.get('always_tailor_resume', True)
        self.auto_skip_waiting = parameters.get('auto_skip_waiting', False)
        ...
        logger.debug(f"Parameters set (always_tailor={self.always_tailor_resume})")
    ...
    def start_applying(self):
        ...
        self.easy_applier_component = AIHawkEasyApplier(self.driver, self.resume_path, self.set_old_answers,
                                                          ...
                                                          always_tailor_resume=self.always_tailor_resume,
        ...
        for position, location in searches:
            ...
            try:
                while True:
                    ...
                    if time_left > 0:
                        if self.auto_skip_waiting:
                            logger.debug(f"Auto-skipping wait of {time_left}s")
                        else:
                            try:
                    ...
                    if page_sleep % 5 == 0:
                        ...
                        if self.auto_skip_waiting:
                            logger.debug(f"Auto-sleeping for {sleep_time}s")
                            time.sleep(sleep_time)
                        else:
                            try:
                                user_input = inputimeout(
    ...
    def get_jobs_from_page(self):
        ...
        try:
            try:
                ...
                try:
                    job_results = self.driver.find_element(By.CLASS_NAME, "scaffold-layout__list-container")
                except NoSuchElementException:
                    job_results = self.driver.find_element(By.XPATH, "//div[contains(@class, 'jobs-search-results-list')]")
            
            utils.scroll_slow(self.driver, job_results)
            utils.scroll_slow(self.driver, job_results, step=300, reverse=True)

            # Try to find the list container again or use the parent of list items
            try:
                list_container = self.driver.find_elements(By.CLASS_NAME, 'scaffold-layout__list-container')[0]
            except (NoSuchElementException, IndexError):
                list_container = self.driver.find_element(By.XPATH, "//ul[contains(@class, 'scaffold-layout__list')] | //div[contains(@class, 'scaffold-layout__list')]")

            job_list_elements = list_container.find_elements(By.CLASS_NAME, 'scaffold-layout__list-item')
    ...
    def read_jobs(self):
        ...
        try:
            job_list_elements = self.driver.find_elements(By.CLASS_NAME, 'scaffold-layout__list-container')[0].find_elements(By.CLASS_NAME, 'scaffold-layout__list-item')
        except (NoSuchElementException, IndexError):
            job_list_elements = self.driver.find_elements(By.CLASS_NAME, 'scaffold-layout__list-item')
    ...
    def extract_job_information_from_tile(self, job_tile):
        ...
        try:
            # Apply Method
            footer_text = job_tile.find_element(By.CLASS_NAME, 'job-card-list__footer-wrapper').text
            if "Easy Apply" in footer_text:
