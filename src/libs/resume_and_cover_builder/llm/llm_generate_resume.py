...
class LLMResumer:
    def __init__(self, api_key, strings):
        if cfg.LLM_MODEL_TYPE == 'gemini':
            ...
            self.llm_cheap = LoggerChatModel(
                ChatOpenAI(
                    model_name="gpt-4o-mini", openai_api_key=api_key, temperature=0.4
                )
            )
    ...
    def generate_header(self, data = None) -> str:
        ...
        } if data is None else data.copy()

        # Ensure personal_information is converted to a string/dict if it's a Pydantic model
        if "personal_information" in input_data:
            pi = input_data["personal_information"]
            if hasattr(pi, "model_dump_json"):
                input_data["personal_information"] = pi.model_dump_json()
            elif hasattr(pi, "json"):
                input_data["personal_information"] = pi.json()
            elif hasattr(pi, "dict"):
                 input_data["personal_information"] = str(pi.dict())
