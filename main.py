...
def create_and_run_bot(parameters, llm_api_key):
    try:
        # Initialize local global_config with settings from config.yaml
        global_config.LLM_MODEL_TYPE = parameters['llm_model_type']
        global_config.LLM_MODEL = parameters['llm_model']
...
def main(collect: False, resume: Path = None):
    try:
        ...
        # If no resume provided via CLI, check if one is defined in config.yaml
        if 'resume' not in parameters['uploads'] and parameters.get('resume_path'):
            config_resume = Path(parameters['resume_path'])
            if config_resume.exists():
                parameters['uploads']['resume'] = config_resume
                logger.info(f"Using default resume from config: {config_resume}")
